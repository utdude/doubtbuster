...
def create(request, parent, parent_id):
    ...
    if request.method == 'POST':
        ...
        if(x > 0.90):
            # context = {"abusive" : "Your comment has been deleted as it contains words which are not allowed."}
            # next+="?abs=1"
        ...
        if body:
            if parent == "post":
                ...
                Comment.objects.create(author=request.user, post=parent,validity=1, body=body)
            else:
